...
<body>
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (selectedDate > minAgeDate) {
      ...
      <div class="submit">
        <input type="button" id="pay-button" class="btn" value="Proceed to Payment">
        <input type="submit" id="submit-button" class="btn" name="submit" value="Register" disabled>
      </div>
    </form>
  </section>
  ...
  <script>
    ...
    const razorpayOptions = {
      ...
      amount: 5000, // ₹50 in paise (₹1 = 100 paise)
      currency: "INR",
      name: "Aaradhya Consultancy",
      description: "Registration Fee",
      image: "https://your-logo-url.com/logo.png", // Optional: Replace with your logo URL
      handler: function(response) {
        alert("Payment successful! Payment ID: " + response.razorpay_payment_id);
        window.paymentCompleted = true; // Mark payment as completed
      ...
      modal: {
        ondismiss: function() {
          ...
          window.paymentCompleted = false; // Reset payment status
        }
      }
    };

    // Initialize Razorpay payment on button click
