...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $form_name = "Shop Act Document Submission"; // Define form name here (or dynamic based on form type)

    // Check for existing submission for the same user and entity type
    $checkQuery = "SELECT COUNT(*) AS count FROM shop_act_submissions WHERE user_id = ? AND entity_type = ?";
    ...
    if ($row['count'] > 0) {
        echo "<script>alert('You have already submitted documents for this entity type.'); window.location.href='shopact.php';</script>";
        $checkStmt->close();
        $conn->close();
        exit;
    }
    ...
    $stmt = $conn->prepare(
        "INSERT INTO shop_act_submissions (user_id, entity_type, form_name, aadhar_card_path, pan_card_path, photo_path, 
         partnership_deed_path, certificate_of_incorporation_path, moa_path)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
